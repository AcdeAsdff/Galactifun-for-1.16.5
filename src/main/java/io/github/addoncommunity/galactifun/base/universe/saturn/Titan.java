...
//        AlienBiomeProviderList.put(name, provider);
    }
    ...
    private void init(@Nonnull WorldInfo info) {
        if (this.generator == null) {
            this.generator = new SimplexOctaveGenerator(worldSeed, 8);
...
//                    }
                    ...
                    if (provider.getBiome(AlienWorldInfoList.get("TITAN"),1, z).biome() == TitanBiome.FOREST.biome()) {
                        Location l = Util.getHighestBlockAt(region, x, z);
                        if (region.getBlock(l.getBlockX(), l.getBlockY(), l.getBlockZ()).getType() == Material.WARPED_NYLIUM) {
        ...
        populators.add(new BlockPopulator() {
            ...
            public void populate(@Nonnull World worldInfo, @Nonnull Random random,@Nonnull Chunk region) {
                ...
                if (random.nextDouble() < 0.25) {
                    ...
                    if (provider.getBiome(AlienWorldInfoList.get("TITAN"),x, z).biome() == TitanBiome.FROZEN_FOREST.biome() && region.getBlock(x, 1, z).getType() == Material.WARPED_NYLIUM) {
