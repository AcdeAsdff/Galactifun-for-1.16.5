...
public abstract class AlienWorld extends PlanetaryWorld {
    ...
    public AlienWorld(String name, PlanetaryType type, Orbit orbit, StarSystem orbiting, ItemStack baseItem,
        ...
        try{
            ...
            this.name = name.replace("&c", "").replace("&6", "").replace(" ", "_").replace("&b", "").toUpperCase();
            this.worldInfo = new WorldInfo() {
                ...
                public void setWorldName(String setname) {
                    this.WorldName = setname;
                }

                @Override
                public @NotNull UUID getUID() {
                    return Bukkit.getWorld("world_galactifun_"+WorldName.toLowerCase()).getUID();
                }

                @Override
                public @NotNull World.Environment getEnvironment() {
                    return Bukkit.getWorld("world_galactifun_"+WorldName.toLowerCase()).getEnvironment();
                }

                @Override
                public long getSeed() {
//                return AlienWorldList.get(WorldName).getSeed();
                    return Bukkit.getWorld("world").getSeed();
                }

                @Override
                public int getMinHeight() {
//                    return AlienWorldList.get(WorldName).getMinHeight();
                    return 1;
                }

                @Override
                public int getMaxHeight() {
//                    return AlienWorldList.get(WorldName).getMaxHeight();
                    return 255;
                }

                @Override
                public BiomeProvider getBiomeProvider() {
                    return AlienBiomeProviderList.get(WorldName);
                }
            };
            this.worldInfo.setWorldName(this.name);
//            System.out.println("worldinfo added" + this.name);
//            AlienWorldInfoList.put(this.name, this.worldInfo);
//            System.out.println("worldinfo added" + this.name);
//        AlienWorldInfoList.notifyAll();
...
//            System.out.println(this.name);
    ...
    public String getAlienWorldName() {
        return CN_name;
    }
...
//            System.out.println("Spawning " + worldName);
            try {
                WorldCreator worldCreator = new WorldCreator(worldName)
                        .environment(atmosphere().environment())
                        .generator(replaceChunkGenerator(new ownChunkGenerator().setName(name().toUpperCase().replace(" ", "_"))
//                {
//
//
//                    /*
...
//                    AlienWorldList.put(name, world);
//                    world = AlienWorldList.get(name);
                } catch (Exception e) {
                    e.printStackTrace();
                }
//                System.out.println("Creating World_");
