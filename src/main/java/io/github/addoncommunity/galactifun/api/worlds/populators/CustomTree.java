...
public class CustomTree { // TODO make this into a populator? or move class
     ...
     */
    public void generate(@Nonnull Chunk accessor, @Nonnull Location location) {
        for (int y = 0; y < this.trunkHeight; y++) {
            location.add(0, 1, 0);
            accessor.getBlock(location.getBlockX(), location.getBlockY(), location.getBlockZ()).setType(this.log);
        }
