...
public class OrePopulator extends BlockPopulator {
    ...
    public void populate(@Nonnull World worldInfo, @Nonnull Random random, @Nonnull Chunk region) {
        int startX = region.getX() << 4;
        int startZ = region.getZ() << 4;
        ...
        for (int i = 0; i < this.attempts; i++) {
            if (random.nextInt(100) < this.chance) {
                ...
                int length = 0;
                while (length < this.maxSize && this.replaceable.contains(region.getBlock(x, y, z).getType())) {
                    region.getBlock(x, y, z).setType( this.ore);
